...
title: "Owen MemTool Product Design"
status: adopted
product: "MemTools"
surface: "PowerShell WPF diagnostic utility"
kind: desktop
updated: 2026-08-12
...
# Owen MemTool Product Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | Windows memory pressure와 driver·kernel 후보를 조사하는 관리자와 지원 엔지니어 |
| Output job | 관찰 시간을 선택해 진단을 실행하고 confidence가 명시된 JSON·Markdown report를 얻는다 |
| Primary object | memory telemetry와 evidence-based diagnostic finding |
| Primary action | 관찰 시간을 선택하고 Start Diagnostics를 실행한다 |
| Composition | Compact diagnostic workbench + progress + evidence table |
| 핵심 판단 | 진단 confidence와 evidence를 강조하고 자동 삭제·비활성화 같은 destructive fix는 제공하지 않는다 |

## Context

MemTools는 Windows 기본 PowerShell 5.1에서 동작하는 local diagnostic tool이다. 사용률 수치 하나가 아니라 physical, commit, kernel pool, process, GPU, driver 근거를 구분한다.
...
- src/MemTools.xaml
- src/Owen-MemTool.ps1
- src/Collect-MemoryDiagnostics.ps1
- src/MemTools.Core.psm1

### Evidence

- README.md
- docs/DIAGNOSTIC-METHODOLOGY.ko.md
- tests/MemTools.Core.Tests.ps1
- tests/Owen-MemTool.Smoke.Tests.ps1

## Foundation Selection

### Adopted

- Duration radio group와 명확한 helper text
- Collection phase와 elapsed time을 실제 progress로 표시
- Confidence badge는 text·icon·color를 함께 사용
- Result body는 sortable opaque table

### Excluded Or App-Owned

- 실제 progress를 모르는 fake percentage
- 진단 결과만으로 driver 삭제·service disable
- process 합계와 RAM 차이를 leak로 단정

## Design Decisions

### D-01. Duration은 목적과 함께 선택한다

- **결정**: 15초, 1분, 5분 옵션에 Quick check, driver analysis, trend watch 설명을 붙인다
- **이유**: 시간 비용과 진단 신뢰도의 tradeoff를 선택 전에 알려야 한다
- **검증**: Keyboard radio path와 선택별 collection duration을 확인한다

### D-02. Confidence는 evidence hierarchy다

- **결정**: 관측됨, 추정, 가능성 높음, 확정을 text와 icon으로 구분한다
- **이유**: 사용자가 추론을 사실로 오해하지 않게 한다
- **검증**: Report와 WPF summary의 confidence label이 일치해야 한다

### D-03. Report location을 완료 상태에 포함한다

- **결정**: 완료 후 Open Report, Copy Path와 JSON·Markdown 위치를 표시한다
- **이유**: 진단의 output job은 결과 파일을 검토하는 데서 끝난다
- **검증**: 저장 실패·partial report·open command를 smoke test한다
...
| 범위 | 유지·변경 | 금지 |
| --- | --- | --- |
| Wide window | navigation·work plane·inspector 책임을 분리 | dense body를 card grid로 바꾸지 않는다 |
| Narrow window | inspector와 보조 panel을 접거나 아래로 이동 | 본문 font와 target을 축소하지 않는다 |
| Minimum size | primary action과 recovery path를 유지 | horizontal overflow와 잘린 dialog를 허용하지 않는다 |
...
- Pester unit tests
- Owen-MemTool WPF smoke tests
- 15sec/1min/5min report schema 비교
